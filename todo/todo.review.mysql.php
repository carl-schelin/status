...
# Description: Retrieve data and update the database with the new info. Prepare and display the table
  ...
  if (isset($_SESSION['username'])) {
    ...
    if (check_userlevel($AL_User)) {
      ...
      logaccess($_SESSION['username'], "todo.review.mysql.php", "Accessing script " . $formVars['id'] . ": week=" . $formVars['week'] . " user=" . $formVars['user'] . " group=" . $formVars['group'] . " save=" . $formVars['save']);

#### Save incoming data if any.

      if ($formVars['save'] >= 0) {

        logaccess($_SESSION['username'], "todo.review.mysql.php", "Updating todo record " . $formVars['id'] . ": save=" . $formVars['save']);
