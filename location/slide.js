...
function init_slide_hide() {
  ...
  for (var i = 0; i < els.length; i++) {
    if (els[i].className == 'hide_me') 
      slide(els[i].id,0);
    if (els[i].className == 'slide_in') { 
      slide(els[i].id,0); 
      slide(els[i].id);
    }
  }
...
function slide(id,duration,step_duration){
  ...
  if (!el || !el.style || (el.morph_data && el.morph_data.timeout))
    return;
...
if (window['addEventListener'])
  addEventListener('load',init_slide_hide,false);
else
  if (window['attachEvent'])
    attachEvent('onload',init_slide_hide);
