<?php
  ...
  if (isset($_GET['search']) && $_GET['search'] != '') {
    ...
    if ($c_st_users == 0) {
      ...
      while ($a_st_users = mysqli_fetch_array($q_st_users)) {
        echo "<div class='suggest_link'><a href='user_edit.php?uid=" . $a_st_users['usr_id'] . "'>" . $a_st_users['suggest'] . "</a></div>\n";
      }
    }
  }
  ...
  switch($do) {
    ...
    case 'check_username_exists': 
      ...
      $c_st_users = mysqli_num_rows(mysqli_query($db, "select * from st_users where usr_name='" . $username . "'"));
    ...
    case 'check_level_exists': 
      ...
      $c_levels = mysqli_num_rows(mysqli_query($db, "select * from st_levels where lvl_name='" . $level . "'"));
