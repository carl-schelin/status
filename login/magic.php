...
if (isset($_POST['mail_user'])) {
  ...
  if (mysqli_num_rows($q_st_users) == '1') {
    ...
    $user_email = $a_st_users['usr_email'];
    ...
    if (isEmail($user_email)) {
      ...
      $q_string .= "where usr_id = " . $a_st_users['usr_id'];
      $q_newpw = mysqli_query($db, $q_string) or die(header("Location: " . $Siteroot . "/error.php?script=" . $package . "&error=" . $q_string . "&mysql=" . mysqli_error($db)));

      $headers  = "From: Status Magic Link <root@" . $Sitehttp . ">\r\n";
      $headers .= "MIME-Version: 1.0\r\n";
      $headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";

      $body  = "<html>";

      $body .= "<p>Here is your Magic Link. Click to autologin to the Status App. This link will expire in 15 minutes.</p>\n\n";

#      $body .= "<p><a href=\"" . $Siteroot . "/index.php?magickey=" . $magickey . "\">Magic Link!</a></p>\n\n";
      $body .= "<p>" . $Siteroot . "/index.php?magickey=" . $magickey . "</p>\n\n";

      $body .= "<p>Note: If you didn't request a Magic Link, there is no impact to your ability to log in or use the Status. The key information will expire as normal.</p>\n\n";
      ...
      print "<p>Click <a href=\"" . $Siteroot . "/index.php\">here</a> to return to the Status site.</p>";
